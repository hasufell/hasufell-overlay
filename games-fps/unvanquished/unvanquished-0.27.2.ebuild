...
SRC_URI="https://github.com/${MY_PN}/${MY_PN}/tarball/v${PV}
	-> ${P}.tar.gz
	x86? ( http://vanquished.zapto.org/~amanieu/unvanquished_deps/linux32-1.tar.bz2 -> unvanquished-${PV}-external-x86.tar.bz2 )
	amd64? ( http://vanquished.zapto.org/~amanieu/unvanquished_deps/linux64-1.tar.bz2 -> unvanquished-${PV}-external-amd64.tar.bz2 )"
...
src_unpack() {
	...
	mv "linux$(usex amd64 "64" "32")-1" "${S}"/external_deps/ || die
}
